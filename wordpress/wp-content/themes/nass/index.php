<?php
get_header();
?>


<?php

if (have_rows('slide_show')) : ?>
	<!--Carousel Wrapper-->
	<div id="carousel-example-2" class="carousel slide carousel-fade main-carousel" data-ride="carousel">
		<!--Indicators-->
		<ol class="carousel-indicators">
			<?php
			$dataSlideId = 0;

			while (have_rows('slide_show')) : the_row();
			?>
				<li data-target="#carousel-example-2" data-slide-to="<?php echo $dataSlideId; ?>"></li>


			<?php $dataSlideId++;
			endwhile; ?>
			<!-- <li data-target="#carousel-example-2" data-slide-to="1"></li>
			<li data-target="#carousel-example-2" data-slide-to="2"></li> -->
		</ol>
		<!--/.Indicators-->
		<!--Slides-->
		<div class="carousel-inner" role="listbox">
			<?php

			while (have_rows('slide_show')) : the_row();
				$slideShowImage = get_sub_field('slide_show_image');
				$slideShowTitle = get_sub_field('slide_show_title');
				$slideShowText = get_sub_field('slide_show_text');
				$slideShowTextButton = get_sub_field('slide_show_text_button');


			?>
				<div class="carousel-item">
					<div class="view">
						<?php if (isset($slideShowImage['sizes']['image_home'])) {
						?>
							<img class="d-block w-100" src="<?php echo $slideShowImage['sizes']['image_home']; ?>" alt="<?php echo $slideShowTitle; ?>" />
						<?php
						} else {
						?>
							<img class="d-block w-100" src="<?php echo bloginfo('template_directory') ?>/img/blank.jpg" alt="<?php echo $slideShowTitle; ?>" />

						<?php
						}
						?>
						<div class="mask rgba-black-light"></div>
					</div>
					<div class="carousel-caption">
						<h1 class="h1-responsive" style="font-weight: 100;"><?php echo $slideShowTitle; ?></h1>
						<p>
							<?php echo $slideShowText;	 ?>
						</p>
						<?php if ($slideShowTextButton) : ?>
							<a href="#" class="btn btn-primary rounded-pill my-2">Learn More</a>
						<?php endif; ?>
						<br /><br />

					</div>

				</div>
			<?php endwhile; ?>

			<!-- <div class="carousel-item">
					<div class="view">
						<img class="d-block w-100" src="<?php// bloginfo('template_directory') ?>/img/3.jpg" alt="Second slide" />
						<div class="mask rgba-black-light"></div>
					</div>
					<div class="carousel-caption">
						<h1 class="h1-responsive">Strong mask</h1>
						<p>Secondary text</p>
					</div>
				</div>
				<div class="carousel-item">
					<div class="view">	
						<img class="d-block w-100" src="<?php //bloginfo('template_directory') 
														?>/img/2.jpg" alt="Third slide" />
						<div class="mask rgba-black-light"></div>
					</div>
					<div class="carousel-caption">
						<h1 class="h1-responsive">Slight mask</h1>
						<p>Third text</p>
					</div>
				</div> -->
		</div>
		<!--/.Slides-->
		<!--Controls-->
		<a class="carousel-control-prev" href="#carousel-example-2" role="button" data-slide="prev">
			<span class="carousel-control-prev-icon" aria-hidden="true"></span>
			<span class="sr-only">Previous</span>
		</a>
		<a class="carousel-control-next" href="#carousel-example-2" role="button" data-slide="next">
			<span class="carousel-control-next-icon" aria-hidden="true"></span>
			<span class="sr-only">Next</span>
		</a>
		<!--/.Controls-->
	</div>
<?php endif; ?>
<!--/.Carousel Wrapper-->

<!--About Us-->
<?php $introLink = get_field('introduction_link'); ?>
<div class="section about-us pt-60 pb-60">
	<div class="container">
		<div class="row">
			<div class="col-xl-6 d-none d-xl-block about-image">
				<img src="<?php show_post('introduction', 'featuredImage'); ?>" width="100%" class="img-responsive about-image hidden-xs hidden-sm" />
			</div>
			<div class="col-xl-6 about-description">
				<h1> <?php show_post('introduction', 'title'); ?>
				</h1>
				<p>
					<?php
					show_post('introduction', 'content');
					?>

				</p>
				<a href="<?php echo $introLink; ?>" class="btn btn-primary rounded-pill my-2">Read More</a>
			</div>
		</div>
	</div>
</div>
<!--About Us-->

<?php if (have_rows('academic_programs')) : ?>
	<!--Academic Programs-->
	<div class="section bg-light academic-programs pt-60 pb-60">
		<div class="container">
			<div class="row">

				<div class="col-lg-12">
					<h1 class="mb-4">Academic Programs</h1>
					<div class="owl-academic owl-carousel owl-theme">
						<?php

						while (have_rows('academic_programs')) : the_row();
							$academicProgramsImage = get_sub_field('academic_programs_image');
							$academicProgramsTitle = get_sub_field('academic_programs_title');
							$academicProgramsText = get_sub_field('academic_programs_text');
							$academicProgramsShift = get_sub_field('academic_programs_shift');
							$academicProgramsButton = get_sub_field('academic_programs_button');


						?>
							<div class="card mb-2">
								<?php if (isset($academicProgramsImage['sizes']['image_academic_programs'])) {
								?>
									<img class="card-img-top" src="<?php echo $academicProgramsImage['sizes']['image_academic_programs']; ?>" alt="<?php echo $academicProgramsTitle; ?>" />
								<?php
								} else {
								?>
									<img class="d-block w-100" src="<?php echo bloginfo('template_directory') ?>/img/academic_blank.jpg" alt="<?php echo $academicProgramsTitle; ?>" />

								<?php
								}
								?>
								<div class="card-body">
									<h4 class="card-title"><?php echo $academicProgramsTitle; ?></h4>
									<p class="card-text">

										<?php
										$academicProgramsText = wp_trim_words($academicProgramsText, 30, null);

										echo $academicProgramsText; ?>
									</p>
									<div class="d-flex justify-content-between align-items-center">
										<div class="btn-group">
											<?php if ($academicProgramsButton) : ?>
												<a href="<?php echo $academicProgramsButton; ?>" type="button" class="btn btn-primary rounded-pill">
													Read More
												</a>
											<?php endif; ?>
										</div>
										<?php if ($academicProgramsShift) : ?>

											<small class="text-dark"><?php echo "Shift: " . implode($academicProgramsShift, ' & '); ?></small>
										<?php endif; ?>

									</div>
								</div>
							</div>

						<?php endwhile; ?>

						<!-- <div class="card mb-2">
							<img class="card-img-top" src="<?php bloginfo('template_directory') ?>/img/c1.jpg" alt="Card image cap" />
							<div class="card-body">
								<h4 class="card-title">+2 Management</h4>
								<p class="card-text">
									Lorem ipsum dolor sit amet, consectetur adipisicing elit.
									Iure voluptate dolorum ipsa illo suscipit quaerat rem natus!
									Sapiente illum error molestias quidem explicabo deserunt
									enim!

									<br />Shift: Morning & Day
								</p>
								<div class="d-flex justify-content-between align-items-center">
									<div class="btn-group">
										<button type="button" class="btn btn-primary rounded-pill">
											Read More
										</button>
									</div>
									<small class="text-dark">Shift: Morning & Day</small>
								</div>
							</div>
						</div>

						<div class="card mb-2">
							<img class="card-img-top" src="<?php bloginfo('template_directory') ?>/img/c2.jpg" alt="Card image cap" />
							<div class="card-body">
								<h4 class="card-title">+2 Education</h4>
								<p class="card-text">
									Lorem ipsum dolor sit amet, consectetur adipisicing elit.
									Iure voluptate dolorum ipsa illo suscipit quaerat rem natus!
									Sapiente illum error molestias quidem explicabo deserunt
									enim!

									<br />Shift: Morning & Day
								</p>
								<div class="d-flex justify-content-between align-items-center">
									<div class="btn-group">
										<button type="button" class="btn btn-primary rounded-pill">
											Read More
										</button>
									</div>
									<small class="text-dark">Shift: Morning & Day</small>
								</div>
							</div>
						</div>

						<div class="card mb-2">
							<img class="card-img-top" src="<?php bloginfo('template_directory') ?>/img/c3.jpg" alt="Card image cap" />
							<div class="card-body">
								<h4 class="card-title">+2 Humanities</h4>
								<p class="card-text">
									Lorem ipsum dolor sit amet, consectetur adipisicing elit.
									Iure voluptate dolorum ipsa illo suscipit quaerat rem natus!
									Sapiente illum error molestias quidem explicabo deserunt
									enim!

									<br />Shift: Morning & Day
								</p>
								<div class="d-flex justify-content-between align-items-center">
									<div class="btn-group">
										<button type="button" class="btn btn-primary rounded-pill">
											Read More
										</button>
									</div>
									<small class="text-dark">Shift: Morning & Day</small>
								</div>
							</div>
						</div>

						<div class="card mb-2">
							<img class="card-img-top" src="<?php bloginfo('template_directory') ?>/img/c2.jpg" alt="Card image cap" />
							<div class="card-body">
								<h4 class="card-title">Bachelor in Business Studies</h4>
								<p class="card-text">
									Lorem ipsum dolor sit amet, consectetur adipisicing elit.
									Iure voluptate dolorum ipsa illo suscipit quaerat rem natus!
									Sapiente illum error molestias quidem explicabo deserunt
									enim!

									<br />Shift: Morning & Day
								</p>
								<div class="d-flex justify-content-between align-items-center">
									<div class="btn-group">
										<button type="button" class="btn btn-primary rounded-pill">
											Read More
										</button>
									</div>
									<small class="text-dark">Shift: Morning & Day</small>
								</div>
							</div>
						</div>

						<div class="card mb-2">
							<img class="card-img-top" src="<?php bloginfo('template_directory') ?>/img/c2.jpg" alt="Card image cap" />
							<div class="card-body">
								<h4 class="card-title">Bachelor in Education</h4>
								<p class="card-text">
									Lorem ipsum dolor sit amet, consectetur adipisicing elit.
									Iure voluptate dolorum ipsa illo suscipit quaerat rem natus!
									Sapiente illum error molestias quidem explicabo deserunt
									enim!

									<br />Shift: Morning & Day
								</p>

								<div class="d-flex justify-content-between align-items-center">
									<div class="btn-group">
										<button type="button" class="btn btn-primary rounded-pill">
											Read More
										</button>
									</div>
									<small class="text-dark">Shift: Morning & Day</small>
								</div>
							</div>
						</div>

						<div class="card mb-2">
							<img class="card-img-top" src="<?php bloginfo('template_directory') ?>/img/c2.jpg" alt="Card image cap" />
							<div class="card-body">
								<h4 class="card-title">Bachelor in Education in IT</h4>
								<p class="card-text">
									Lorem ipsum dolor sit amet, consectetur adipisicing elit.
									Iure voluptate dolorum ipsa illo suscipit quaerat rem natus!
									Sapiente illum error molestias quidem explicabo deserunt
									enim!

									<br />Shift: Morning & Day
								</p>
								<div class="d-flex justify-content-between align-items-center">
									<div class="btn-group">
										<button type="button" class="btn btn-primary rounded-pill">
											Read More
										</button>
									</div>
									<small class="text-dark">Shift: Morning & Day</small>
								</div>
							</div>
						</div> -->
					</div>
				</div>
			</div>
		</div>
	</div>
<?php endif; ?>
<!--Academic Programs-->

<!--School Features-->
<?php if (have_rows('school_facilities')) : ?>
	<div class="parallax">
		<div class="section school-features pt-60">
			<div class="container">
				<div class="row">

					<div class="col-lg-12">
						<h1 class="mb-4">School Facilities</h1>
					</div>
					<div class="feature-list">
						<ul>

							<?php while (have_rows('school_facilities')) : the_row();
								$schoolFacititiesIcon = get_sub_field('school_facilities_icon');
								$schoolFacilitiesName = get_sub_field('school_facilities_name');
							?>
								<li>
									<?php echo $schoolFacititiesIcon; ?>
									<h5><?php echo $schoolFacilitiesName;
										?></h5>

								</li>
								<li></li>

							<?php endwhile; ?>
						</ul>

						<!-- 
						<ul>
							<li>
								<svg viewBox="0 0 480 480" xmlns="http://www.w3.org/2000/svg">
									<path d="m240 0h-104c-4.417969 0-8 3.582031-8 8v56h-120c-4.417969 0-8 3.582031-8 8v400c0 4.417969 3.582031 8 8 8h232c4.417969 0 8-3.582031 8-8v-464c0-4.417969-3.582031-8-8-8zm-8 416h-88v-16h88zm-216-296h112v240h-112zm216-32h-88v-24h88zm-216 288h112v24h-112zm128 8v-280h88v280zm88-368v32h-88v-32zm-104 64v24h-112v-24zm-112 336h112v48h-112zm128 48v-32h88v32zm0 0" />
									<path d="m479.742188 437.984375-96-368c-.535157-2.0625-1.871094-3.824219-3.714844-4.898437-1.839844-1.070313-4.03125-1.363282-6.089844-.8125l-120 32c-4.25 1.132812-6.789062 5.484374-5.679688 9.742187l96 368c.535157 2.0625 1.875 3.824219 3.71875 4.894531 1.839844 1.066406 4.03125 1.355469 6.085938.800782l120-32c4.242188-1.132813 6.777344-5.476563 5.679688-9.726563zm-201.246094-279.441406 104.542968-27.871094 3.761719 14.398437-104.535156 27.914063zm112.34375 2.050781 46.398437 177.980469-104.503906 27.882812-46.398437-177.992187zm50.472656 193.46875 8.191406 31.394531-104.542968 27.871094-8.160157-31.382813zm-71.023438-272.261719 8.710938 33.398438-104.542969 27.863281-8.703125-33.382812zm-12.578124 380.398438-8.710938-33.398438 104.542969-27.878906 8.703125 33.382813zm0 0" />
									<path d="m104 144h-64c-4.417969 0-8 3.582031-8 8v64c0 4.417969 3.582031 8 8 8h64c4.417969 0 8-3.582031 8-8v-64c0-4.417969-3.582031-8-8-8zm-8 64h-48v-48h48zm0 0" />
								</svg>
								<h5>Resourceful Library</h5>
							</li>
							<li>
								<svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 512 512" style="enable-background: new 0 0 512 512;" xml:space="preserve">
									<g>
										<g>
											<path d="M290.359,432L181.335,263.467V128c0-5.891-4.776-10.667-10.667-10.667h-53.333c-5.891,0-10.667,4.776-10.667,10.667
	v135.765L7.266,432.747c-4.755,8.082-7.263,17.288-7.263,26.666C0.002,488.455,23.546,511.999,52.588,512h194.24
	c9.983-0.003,19.752-2.887,28.136-8.306C299.013,488.147,305.906,456.049,290.359,432z M246.828,490.667H52.588
	c-5.591,0.009-11.083-1.482-15.902-4.318c-14.876-8.754-19.839-27.91-11.085-42.786L126.53,272.075
	c0.964-1.639,1.472-3.506,1.472-5.408v-128h32v128c-0.001,2.055,0.592,4.066,1.707,5.792l110.72,171.125
	c3.194,4.935,4.894,10.687,4.896,16.566C277.33,476.998,263.677,490.661,246.828,490.667z" />
										</g>
									</g>
									<g>
										<g>
											<path d="M153.655,310.112l-18.389-10.805L44.695,453.259c-0.966,1.642-1.475,3.514-1.473,5.419
	c0.006,5.891,4.787,10.662,10.678,10.655h74.101V448H72.535L153.655,310.112z" />
										</g>
									</g>
									<g>
										<g>
											<path d="M486.252,245.333H320.002c-23.564,0-42.667,19.103-42.667,42.667v170.667c0.035,29.441,23.893,53.298,53.333,53.333h96
	c29.441-0.035,53.298-23.893,53.333-53.333V313.749l24.469-24.459c4.817-4.826,7.523-11.365,7.527-18.183
	C512.006,256.881,500.479,245.341,486.252,245.333z M489.378,274.208l-27.584,27.584c-2,2-3.125,4.713-3.125,7.541v149.333
	c0,17.673-14.327,32-32,32h-96c-17.673,0-32-14.327-32-32V288c0-11.782,9.551-21.333,21.333-21.333l166.251,0
	c1.174-0.001,2.301,0.465,3.131,1.296C491.106,269.689,491.104,272.485,489.378,274.208z" />
										</g>
									</g>
									<g>
										<g>
											<path d="M416.002,309.333V448h-32v21.333h42.667c5.891,0,10.667-4.776,10.667-10.667V309.333H416.002z" />
										</g>
									</g>
									<g>
										<g>
											<path d="M298.668,154.496V10.667C298.668,4.776,293.893,0,288.002,0h-74.667c-5.891,0-10.667,4.776-10.667,10.667v146.859
	c-13.018,4.008-25.541,9.472-37.333,16.288l10.667,18.464c12.532-7.236,25.988-12.737,40-16.352c4.712-1.217,8.004-5.469,8-10.336
	V21.333h53.333v141.771c0.002,5.12,3.642,9.517,8.672,10.475c37.402,7.15,71.033,27.393,94.859,57.099l16.629-13.355
	C372.394,186.042,337.644,163.951,298.668,154.496z" />
										</g>
									</g>
									<g>
										<g>
											<path d="M453.335,149.333c-20.619,0-37.333,16.715-37.333,37.333c0,20.619,16.715,37.333,37.333,37.333
	c20.619,0,37.333-16.715,37.333-37.333C490.668,166.048,473.954,149.333,453.335,149.333z M453.335,202.667
	c-8.837,0-16-7.163-16-16s7.163-16,16-16s16,7.163,16,16S462.172,202.667,453.335,202.667z" />
										</g>
									</g>
									<g>
										<g>
											<path d="M416.002,74.667c-17.673,0-32,14.327-32,32c0,17.673,14.327,32,32,32s32-14.327,32-32
	C448.002,88.994,433.675,74.667,416.002,74.667z M416.002,117.333c-5.891,0-10.667-4.776-10.667-10.667S410.111,96,416.002,96
	c5.891,0,10.667,4.776,10.667,10.667S421.893,117.333,416.002,117.333z" />
										</g>
									</g>
									<g>
										<g>
											<path d="M37.335,85.333c-20.619,0-37.333,16.715-37.333,37.333C0.002,143.285,16.716,160,37.335,160
	c20.619,0,37.333-16.715,37.333-37.333C74.668,102.048,57.954,85.333,37.335,85.333z M37.335,138.667c-8.837,0-16-7.163-16-16
	s7.163-16,16-16c8.837,0,16,7.163,16,16S46.172,138.667,37.335,138.667z" />
										</g>
									</g>
									<g>
										<g>
											<path d="M85.335,10.667c-17.673,0-32,14.327-32,32c0,17.673,14.327,32,32,32c17.673,0,32-14.327,32-32
	C117.335,24.994,103.008,10.667,85.335,10.667z M85.335,53.333c-5.891,0-10.667-4.776-10.667-10.667
	C74.668,36.776,79.444,32,85.335,32c5.891,0,10.667,4.776,10.667,10.667C96.002,48.558,91.226,53.333,85.335,53.333z" />
										</g>
									</g>
									<g>
										<g>
											<rect x="320.002" y="309.333" width="32" height="21.333" />
										</g>
									</g>
									<g>
										<g>
											<rect x="320.002" y="352" width="21.333" height="21.333" />
										</g>
									</g>
									<g>
										<g>
											<rect x="320.002" y="394.667" width="32" height="21.333" />
										</g>
									</g>
									<g>
										<g>
											<rect x="320.002" y="437.333" width="21.333" height="21.333" />
										</g>
									</g>
									<g>
										<g>
											<rect x="21.335" y="192" width="21.333" height="21.333" />
										</g>
									</g>
									<g>
										<g>
											<rect x="53.335" y="245.333" width="21.333" height="21.333" />
										</g>
									</g>
									<g>
										<g>
											<rect x="234.668" y="288" width="21.333" height="21.333" />
										</g>
									</g>
									<g>
										<g>
											<rect x="213.335" y="245.333" width="21.333" height="21.333" />
										</g>
									</g>
									<g>
										<g>
											<rect x="202.668" y="202.667" width="21.333" height="21.333" />
										</g>
									</g>
									<g>
										<g>
											<rect x="213.335" y="437.333" width="21.333" height="21.333" />
										</g>
									</g>
									<g>
										<g>
											<rect x="181.335" y="394.667" width="21.333" height="21.333" />
										</g>
									</g>
									<g></g>
									<g></g>
									<g></g>
									<g></g>
									<g></g>
									<g></g>
									<g></g>
									<g></g>
									<g></g>
									<g></g>
									<g></g>
									<g></g>
									<g></g>
									<g></g>
									<g></g>
								</svg>

								<h5>Science Laboratory</h5>
							</li>
							<li>
								<svg viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg">
									<path d="m316 346c-5.519531 0-10 4.480469-10 10s4.480469 10 10 10 10-4.480469 10-10-4.480469-10-10-10zm0 0" />
									<path d="m462 0h-412c-27.570312 0-50 22.429688-50 50v306c0 27.570312 22.429688 50 50 50h156v46h-10c-13.351562 0-25.902344 5.203125-35.351562 14.648438-9.445313 9.449218-14.648438 22-14.648438 35.351562 0 5.523438 4.476562 10 10 10h200c5.523438 0 10-4.476562 10-10 0-27.570312-22.429688-50-50-50h-10v-46h156c27.570312 0 50-22.429688 50-50v-306c0-27.570312-22.429688-50-50-50zm-412 20h412c16.542969 0 30 13.457031 30 30v256h-46v-230c0-5.523438-4.476562-10-10-10h-80c-5.523438 0-10 4.476562-10 10v230h-50.054688c-.105468-10.34375-.375-20.539062-.804687-30.429688-1.75-40.726562-5.738281-68.007812-7.8125-80.007812 20.632813-15.191406 26.265625-43.347656 13.652344-65.222656l-34.660157-60.03125c-1.789062-3.09375-5.089843-5-8.660156-5-3.574218 0-6.875 1.90625-8.660156 5l-34.675781 60.035156c-11.890625 20.625-7.675781 47.339844 11.003906 63.105469-2.164063 11.964843-6.53125 40.222656-8.429687 82.101562-.449219 9.996094-.734376 20.199219-.84375 30.449219h-50.054688v-150c0-1.585938-.375-3.03125-.988281-4.316406-.027344-.050782-.042969-.105469-.066407-.15625l-40-80c-1.695312-3.386719-5.15625-5.527344-8.945312-5.527344s-7.25 2.140625-8.945312 5.527344l-40 80c-.023438.050781-.039063.105468-.0625.15625-.625 1.296875-.992188 2.765625-.992188 4.316406v150h-46v-256c0-16.542969 13.457031-30 30-30zm316 286v-220h60v20h-10c-5.523438 0-10 4.476562-10 10s4.476562 10 10 10h10v30h-10c-5.523438 0-10 4.476562-10 10s4.476562 10 10 10h10v30h-10c-5.523438 0-10 4.476562-10 10s4.476562 10 10 10h10v40h-10c-5.523438 0-10 4.476562-10 10s4.476562 10 10 10h10v20zm-108.339844-100.691406c3.632813 0 7.25-.417969 10.789063-1.199219 1.855469 11.894531 4.582031 32.941406 6.195312 61.890625h-37.222656c1.691406-28.929688 4.546875-50.304688 6.519531-62.601562 4.429688 1.261718 9.042969 1.910156 13.71875 1.910156zm0-20c-22.855468 0-37.59375-24.875-26.011718-44.96875l26.011718-45.03125 25.996094 45.027344c11.539062 20.011718-2.996094 44.972656-25.996094 44.972656zm-21.15625 100.691406h39.019532c.210937 6.570312.355468 13.253906.425781 20h-39.894531c.074218-6.703125.226562-13.386719.449218-20zm-110.503906 20v-140h20v140zm-40 0v-140h20v140zm6.179688-160 23.820312-47.640625 23.820312 47.640625zm252.105468 346h-176.582031c1.46875-4.167969 3.867187-7.988281 7.085937-11.210938 5.671876-5.667968 13.203126-8.789062 21.210938-8.789062h120c13.039062 0 24.160156 8.359375 28.285156 20zm-58.285156-40h-60v-46h60zm176-66h-412c-16.542969 0-30-13.457031-30-30v-30h472v30c0 16.542969-13.457031 30-30 30zm0 0" />
									<path d="m276 346h-60c-5.523438 0-10 4.476562-10 10s4.476562 10 10 10h60c5.523438 0 10-4.476562 10-10s-4.476562-10-10-10zm0 0" />
								</svg>
								<h5>Computer Laboratory</h5>
							</li>
							<li>
								<svg viewBox="0 0 496.00151 496" xmlns="http://www.w3.org/2000/svg">
									<path d="m488 320c4.414062 0 8-3.574219 8-8v-304c0-4.421875-3.585938-8-8-8h-480c-4.414062 0-8 3.578125-8 8v304c0 4.425781 3.585938 8 8 8h133.527344l-45.246094 165.898438c-.65625 2.398437-.152344 4.972656 1.359375 6.957031 1.519531 1.976562 3.863281 3.144531 6.359375 3.144531h32c3.609375 0 6.769531-2.414062 7.71875-5.894531l46.394531-170.105469h33.886719v168c0 4.425781 3.585938 8 8 8h32c4.414062 0 8-3.574219 8-8v-168h34.230469l26.722656 80.152344c-25.050781 1.585937-44.953125 22.410156-44.953125 47.847656 0 26.472656 21.527344 48 48 48h152c3.230469 0 6.152344-1.941406 7.390625-4.933594 1.242187-2.992187.554687-6.433594-1.734375-8.722656l-26.335938-26.34375c-2.144531-2.144531-3.320312-4.976562-3.320312-8s1.175781-5.855469 3.3125-8l26.34375-26.34375c2.289062-2.289062 2.976562-5.726562 1.734375-8.71875-1.238281-2.992188-4.160156-4.9375-7.390625-4.9375h-106.230469l-26.664062-80zm-472-304h464v288h-464zm113.886719 464h-15.414063l43.640625-160h15.414063zm126.113281 0h-16v-160h16zm48-32c0-17.648438 14.351562-32 32-32v64c-17.648438 0-32-14.351562-32-32zm152-19.3125c-5.160156 5.167969-8 12.023438-8 19.3125s2.839844 14.152344 8 19.3125l12.6875 12.6875h-116.6875v-24h64v-16h-64v-24h116.6875zm-91.105469-28.6875h-15.132812l-26.664063-80h15.132813zm0 0" />
									<path d="m456 32h-416c-4.414062 0-8 3.578125-8 8v240c0 4.425781 3.585938 8 8 8h416c4.414062 0 8-3.574219 8-8v-240c0-4.421875-3.585938-8-8-8zm-56 240h-48v-16h48zm48 0h-32v-24c0-4.421875-3.585938-8-8-8h-64c-4.414062 0-8 3.578125-8 8v24h-288v-224h400zm0 0" />
									<path d="m113.574219 229.898438c.335937-.226563.707031-.265626 1.097656-.128907.382813.160157.632813.429688.734375.832031 1.648438 6.582032 6.816406 11.464844 13.480469 12.726563 6.640625 1.265625 13.257812-1.367187 17.203125-6.894531l18.910156-26.480469c.105469-.144531.441406-.617187 1.113281-.496094.695313.078125.910157.558594.976563.710938l10.871094 25.375c2.488281 5.808593 7.941406 9.769531 14.230468 10.347656 6.328125.535156 12.382813-2.335937 16.234375-8.121094.207031-.3125.511719-.496093.886719-.519531.390625-.050781.695312.078125.96875.34375l10.421875 10.421875c6 6.007813 15.671875 6.6875 22.480469 1.59375l25.617187-19.207031-9.601562-12.800782-25.613281 19.207032c-.488282.351562-1.152344.296875-1.570313-.113282l-10.414063-10.40625c-3.65625-3.664062-8.730468-5.457031-13.847656-4.96875-5.152344.503907-9.761718 3.265626-12.984375 8.113282-.097656.144531-.378906.511718-1.113281.527344-.710938-.070313-.929688-.558594-.992188-.71875l-10.871093-25.378907c-2.488281-5.78125-7.703125-9.636719-13.960938-10.308593-6.273437-.699219-12.175781 1.972656-15.847656 7.101562l-18.921875 26.488281c-.117188.167969-.4375.609375-1.191406.480469-.75-.144531-.886719-.6875-.941406-.886719-1.355469-5.425781-5.28125-9.859375-10.488282-11.875-5.210937-2.015625-11.105468-1.367187-15.746094 1.738281l-37.136718 24.757813 8.875 13.3125zm0 0" />
									<path d="m72 160h184v16h-184zm0 0" />
									<path d="m72 96h184v16h-184zm0 0" />
									<path d="m72 128h184v16h-184zm0 0" />
									<path d="m296 216c1 0 2.007812-.183594 2.976562-.566406l40-16c1-.410156 1.917969-1.007813 2.679688-1.777344l88-88c3.128906-3.128906 3.128906-8.183594 0-11.3125l-24-24c-3.128906-3.128906-8.183594-3.128906-11.3125 0l-88 88c-.769531.769531-1.367188 1.679688-1.777344 2.6875l-16 40c-1.183594 2.96875-.488281 6.363281 1.777344 8.625 1.527344 1.527344 3.578125 2.34375 5.65625 2.34375zm48-43.3125-12.6875-12.6875 44.6875-44.6875 12.6875 12.6875zm56-81.375 12.6875 12.6875-12.6875 12.6875-12.6875-12.6875zm-81.183594 81.183594 1.183594-1.183594 12.6875 12.6875-1.183594 1.183594-21.144531 8.464844zm0 0" />
								</svg>
								<h5>Digital Classrooms</h5>
							</li>
							<li>
								<svg viewBox="0 0 60 60" xmlns="http://www.w3.org/2000/svg">
									<path id="Shape" d="m57 7c-1.6568542 0-3 1.34314575-3 3v3.026c-.8603889-.6618067-1.9145307-1.0223232-3-1.026h-5v-6c0-.55228475-.4477153-1-1-1s-1 .44771525-1 1v1h-8v-1c0-.55228475-.4477153-1-1-1s-1 .44771525-1 1v6h-13.278c-.3544126-.6167513-1.0106732-.997855-1.722-1h-.422c-1.0210036-2.34315137-3.6471828-3.53629685-6.0836368-2.76396359s-3.89568634 3.26051619-3.3803632 5.76396359c-1.07055237-.1425406-2.15892456.0517867-3.114.556v-11.556c-.00000004-1.65685422-1.34314578-2.99999992-3-2.99999992s-2.99999996 1.3431457-3 2.99999992v54c0 1.6568542 1.34314575 3 3 3s3-1.3431458 3-3v-5h48v5c0 1.6568542 1.3431458 3 3 3s3-1.3431458 3-3v-47c0-1.65685425-1.3431458-3-3-3zm-40 39v-9h2v9zm-11 0v-4c0-1.1045695.8954305-2 2-2h2.008c1.1544518 1.5475406 3.1026705 2.2835864 4.992 1.886v4.114zm28-22v2h-28v-2zm10-7h-8v-3h8zm-8 2h8v3h-8zm8 5v2h-8v-2zm0 17h-8v-3h8zm-8-5v-3h8v3zm-15.278 0c-.3544126-.6167513-1.0106732-.997855-1.722-1h-.422c-1.0210036-2.3431514-3.6471828-3.5362969-6.0836368-2.7639636s-3.89568634 3.2605162-3.3803632 5.7639636c-1.07055237-.1425406-2.15892456.0517867-3.114.556v-10.556h28v8zm-9.316 2.5c-.7934427-1.3549274-.4049542-3.0926713.8899295-3.9807343 1.2948836-.888063 3.0559109-.6245097 4.0340705.6037343-.7948047.2825815-1.3268615 1.0334617-1.33 1.877v2.815c-1.3581488.4593018-2.8529803-.0876385-3.594-1.315zm32.594-7.5h-8v-3h8zm2-3h8v9.026c-.8603889-.6618067-1.9145307-1.0223232-3-1.026h-5zm8-2h-8v-2h8zm-33 12h13v8h-13zm15 5h8v3h-8zm10-5h5c1.6568542 0 3 1.3431458 3 3v5h-8zm5-24c1.6568542 0 3 1.3431458 3 3v5h-8v-8zm-15-5h8v3h-8zm-2 13h-13v-8h13zm-15 0h-2v-9h2zm-5-12c.9071075.0011737 1.7644636.4147652 2.33 1.124-.7944608.2824573-1.3264407 1.032829-1.33 1.876v2.815c-1.3581586.460102-2.8534893-.0870209-3.594-1.315-.5355357-.9275728-.5359247-2.0702995-.0010205-2.9982367.5349043-.9279371 1.523952-1.5003086 2.5950205-1.5017633zm-6 6h2.008c1.1544518 1.5475406 3.1026705 2.2835864 4.992 1.886v4.114h-9v-4c0-1.1045695.8954305-2 2-2zm-4 41c0 .5522847-.44771525 1-1 1s-1-.4477153-1-1v-54c0-.55228475.44771525-1 1-1s1 .44771525 1 1zm2-9h28v2h-28zm30 2v-2h8v2zm10 0v-2h8v2zm12 7c0 .5522847-.4477153 1-1 1s-1-.4477153-1-1v-47c0-.55228475.4477153-1 1-1s1 .44771525 1 1z" />
								</svg>
								<h5>School Hostel</h5>
							</li>
							<li>
								<svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 512.001 512.001" style="enable-background: new 0 0 512.001 512.001;" xml:space="preserve">
									<g>
										<g>
											<path d="M264.65,2.93C262.79,1.07,260.21,0,257.58,0s-5.21,1.069-7.07,2.93c-1.86,1.861-2.93,4.44-2.93,7.07s1.07,5.21,2.93,7.069
	c1.86,1.861,4.44,2.931,7.07,2.931s5.2-1.07,7.07-2.931c1.86-1.859,2.93-4.439,2.93-7.069S266.51,4.79,264.65,2.93z" />
										</g>
									</g>
									<g>
										<g>
											<path d="M492,96.642v-28.15c0-15.168-12.34-27.508-27.508-27.508H447.13C442.871,17.706,422.451,0,397.96,0h-97.626
	c-5.523,0-10,4.478-10,10s4.477,10,10,10h97.626c16.542,0,30,13.458,30,30v30H332V50c0-5.522-4.477-10-10-10H190
	c-5.523,0-10,4.478-10,10v30H84.131V50c0-16.542,13.458-30,30-30H214.5c5.523,0,10-4.478,10-10s-4.477-10-10-10H114.131
	c-24.49,0-44.911,17.705-49.17,40.984H47.508C32.34,40.984,20,53.324,20,68.492v28.15c-11.639,4.128-20,15.243-20,28.28v37.369
	c0,16.542,13.458,30,30,30s30-13.458,30-30v-37.369c0-13.036-8.361-24.152-20-28.28v-28.15c0-4.14,3.368-7.508,7.508-7.508h16.624
	V393c0,20.086,11.912,37.433,29.037,45.376V474c0,20.953,17.047,38,38,38s38-17.047,38-38v-31h173.918v31
	c0,20.953,17.047,38,38,38s38-17.047,38-38v-35.697c17.038-7.978,28.874-25.28,28.874-45.303V60.984h16.532
	c4.14,0,7.508,3.368,7.508,7.508v28.15c-11.639,4.128-20,15.243-20,28.28v37.369c0,16.542,13.458,30,30,30s30-13.458,30-30
	v-37.369C512,111.885,503.639,100.771,492,96.642z M40,162.292c0,5.514-4.486,10-10,10s-10-4.486-10-10v-37.369
	c0-5.514,4.486-10,10-10s10,4.486,10,10V162.292z M326.644,100l-31.151,31.151c-3.905,3.905-3.905,10.237,0,14.143
	c1.953,1.952,4.512,2.929,7.071,2.929s5.119-0.977,7.071-2.929L354.929,100h58.84L266.004,247.764V100H326.644z M200,60h112v20
	H200V60z M203.282,234.6c-3.905-3.903-10.236-3.904-14.143,0l-62.159,62.159h-6.409l-30.927-30.927l156.36-156.36v187.287h-90.739
	l48.017-48.017C207.187,244.837,207.188,238.505,203.282,234.6z M84.131,100h143.06L84.131,243.06V100z M149.168,474
	c0,9.925-8.075,18-18,18c-9.925,0-18-8.075-18-18v-31h0.963h35.037V474z M399.086,474c0,9.925-8.075,18-18,18s-18-8.075-18-18v-31
	h34.874h1.126V474z M427.96,393c0,16.542-13.458,30-30,30H114.131c-16.542,0-30-13.458-30-30V288.604l25.227,25.227
	c1.875,1.875,4.419,2.929,7.071,2.929h279.142c2.652,0,5.196-1.054,7.071-2.929l25.318-25.318V393z M427.96,260.228
	l-36.531,36.531H266.004v-20.71L427.96,114.094V260.228z M492,162.292c0,5.514-4.486,10-10,10c-5.514,0-10-4.486-10-10v-37.369
	c0-5.514,4.486-10,10-10c5.514,0,10,4.486,10,10V162.292z" />
										</g>
									</g>
									<g>
										<g>
											<path d="M140.044,339.88c-16.542,0-30,13.458-30,30s13.458,30,30,30s30-13.458,30-30S156.586,339.88,140.044,339.88z
	 M140.044,379.88c-5.514,0-10-4.486-10-10c0-5.514,4.486-10,10-10c5.514,0,10,4.486,10,10
	C150.044,375.394,145.558,379.88,140.044,379.88z" />
										</g>
									</g>
									<g>
										<g>
											<path d="M371.956,339.88c-16.542,0-30,13.458-30,30s13.458,30,30,30s30-13.458,30-30S388.498,339.88,371.956,339.88z
	 M371.956,379.88c-5.514,0-10-4.486-10-10c0-5.514,4.486-10,10-10c5.514,0,10,4.486,10,10
	C381.956,375.394,377.47,379.88,371.956,379.88z" />
										</g>
									</g>
									<g>
										<g>
											<path d="M308.083,339.88h-104c-5.523,0-10,4.478-10,10v40c0,5.522,4.477,10,10,10h104c5.523,0,10-4.478,10-10v-40
	C318.083,344.357,313.606,339.88,308.083,339.88z M298.083,379.88h-84v-20h84V379.88z" />
										</g>
									</g>
									<g></g>
									<g></g>
									<g></g>
									<g></g>
									<g></g>
									<g></g>
									<g></g>
									<g></g>
									<g></g>
									<g></g>
									<g></g>
									<g></g>
									<g></g>
									<g></g>
									<g></g>
								</svg>

								<h5>Efficient Transporation</h5>
							</li>
							<li>
								<svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 496.592 496.592" xml:space="preserve">
									<g>
										<g>
											<path d="M400.296,328v-51.696l-21.816,8.488l-0.064-0.144l-0.52-0.944c-8.36-13.016-21.88-22.072-37.104-24.832l-52.496-9.552
	v-4.144c19.632-11.4,34.008-30.848,38.488-53.84c14.544-3.008,25.512-15.912,25.512-31.336V80.984
	c0-22.328-14.624-41.8-35.84-48.136l-4-4C293.848,10.24,269.112,0,242.8,0c-54.312,0-98.504,44.184-98.504,98.504V160
	c0,15.424,10.968,28.328,25.512,31.336c4.488,22.992,18.856,42.448,38.488,53.84v4.144l-52.488,9.544
	c-15.208,2.76-28.72,11.8-37.088,24.808l-0.48,1.168l-21.944-8.536V328c-13.232,0-24,10.768-24,24v32c0,13.232,10.768,24,24,24
	v29.472l152,59.12l152-59.12V408c13.232,0,24-10.768,24-24v-32C424.296,338.768,413.528,328,400.296,328z M303.288,268.312
	l34.64,6.296c10.2,1.856,19.232,7.744,25.256,16.128l-77.4,30.096L303.288,268.312z M224.976,252.512
	c7.384,2.256,15.208,3.488,23.32,3.488s15.936-1.232,23.32-3.488L248.296,270L224.976,252.512z M236.12,280.864l-16.328,16.32
	L209.2,265.416l5.096-0.92L236.12,280.864z M282.296,264.504l5.096,0.92L276.8,297.192l-16.328-16.32L282.296,264.504z
	 M328.296,173.776v-27.552c4.76,2.776,8,7.88,8,13.776S333.056,171,328.296,173.776z M168.296,173.776
	c-4.76-2.776-8-7.88-8-13.776s3.24-11,8-13.776V173.776z M168.296,120v9.136c-2.848,0.744-5.52,1.864-8,3.312V98.504
	C160.296,53.016,197.304,16,242.8,16c22.032,0,42.76,8.584,58.344,24.168l7.056,7.064l2.152,0.528
	c15.28,3.816,25.944,17.48,25.944,33.224v51.464c-2.48-1.448-5.152-2.576-8-3.312V120h-25.368c-21.68,0-42.4-10.352-55.4-27.688
	l-6.36-8.496L225.272,99.72C212.184,112.8,194.792,120,176.296,120H168.296z M184.296,176v-40.368
	c19.752-1.832,38.08-10.4,52.288-24.6l3.112-3.112c16.04,17.704,39.144,28.08,63.232,28.08h9.368v40c0,35.288-28.712,64-64,64
	C213.008,240,184.296,211.288,184.296,176z M158.664,274.608l34.64-6.296l17.504,52.52l-77.416-30.104
	C139.416,282.336,148.464,276.464,158.664,274.608z M88.296,384v-32c0-4.416,3.592-8,8-8c13.232,0,24,10.768,24,24
	s-10.768,24-24,24C91.888,392,88.296,388.416,88.296,384z M240.296,476.304l-128-49.768v-21.928c14.104-6.192,24-20.248,24-36.608
	c0-16.36-9.896-30.416-24-36.608v-31.696l128,49.776V476.304z M216.544,323.072l31.752-31.76l31.752,31.752l-31.752,12.352
	L216.544,323.072z M384.296,426.528l-128,49.768V349.472l128-49.776v31.696c-14.104,6.192-24,20.248-24,36.608
	c0,16.36,9.896,30.416,24,36.608V426.528z M408.296,384c0,4.416-3.592,8-8,8c-13.232,0-24-10.768-24-24s10.768-24,24-24
	c4.408,0,8,3.584,8,8V384z" />
										</g>
									</g>
									<g></g>
									<g></g>
									<g></g>
									<g></g>
									<g></g>
									<g></g>
									<g></g>
									<g></g>
									<g></g>
									<g></g>
									<g></g>
									<g></g>
									<g></g>
									<g></g>
									<g></g>
								</svg>

								<h5>Day Boarding</h5>
							</li>
							<li>
								<svg viewBox="0 0 60 60" xmlns="http://www.w3.org/2000/svg">
									<g>
										<path id="Shape" d="m42 12c0 3.3137085 2.6862915 6 6 6s6-2.6862915 6-6-2.6862915-6-6-6c-3.3123376.00330738-5.9966926 2.68766243-6 6zm6-4c2.209139 0 4 1.790861 4 4s-1.790861 4-4 4-4-1.790861-4-4 1.790861-4 4-4z" />
										<path id="Shape" d="m49 3v-2c0-.55228475-.4477153-1-1-1s-1 .44771525-1 1v2c0 .55228475.4477153 1 1 1s1-.44771525 1-1z" />
										<path id="Shape" d="m47 21v2c0 .5522847.4477153 1 1 1s1-.4477153 1-1v-2c0-.5522847-.4477153-1-1-1s-1 .4477153-1 1z" />
										<path id="Shape" d="m59 11h-2c-.5522847 0-1 .4477153-1 1s.4477153 1 1 1h2c.5522847 0 1-.4477153 1-1s-.4477153-1-1-1z" />
										<path id="Shape" d="m37 11c-.5522847 0-1 .4477153-1 1s.4477153 1 1 1h2c.5522847 0 1-.4477153 1-1s-.4477153-1-1-1z" />
										<path id="Shape" d="m40.929 3.515c-.3923789-.37897221-1.0160848-.37355237-1.4018186.01218142-.3857338.3857338-.3911536 1.00943969-.0121814 1.40181858l1.414 1.414c.3923789.37897221 1.0160848.37355237 1.4018186-.01218142.3857338-.3857338.3911536-1.00943969.0121814-1.40181858z" />
										<path id="Shape" d="m53.657 17.657c-.3903819.3904999-.3903819 1.0235001 0 1.414l1.414 1.414c.3923789.3789722 1.0160848.3735524 1.4018186-.0121814s.3911536-1.0094397.0121814-1.4018186l-1.414-1.414c-.3904999-.3903819-1.0235001-.3903819-1.414 0z" />
										<path id="Shape" d="m56.485 3.515c-.3904999-.39038194-1.0235001-.39038194-1.414 0l-1.414 1.414c-.2599566.25107447-.3642126.62287789-.2726972.97250732.0915155.34962944.3645604.6226744.7141899.71418984.3496294.09151544.7214328-.01274058.9725073-.27269716l1.414-1.414c.3903819-.39049985.3903819-1.02350015 0-1.414z" />
										<path id="Shape" d="m40.929 17.657-1.414 1.414c-.2599566.2510745-.3642126.6228779-.2726972.9725073.0915155.3496295.3645604.6226744.7141899.7141899.3496294.0915154.7214328-.0127406.9725073-.2726972l1.414-1.414c.3789722-.3923789.3735524-1.0160848-.0121814-1.4018186s-1.0094397-.3911536-1.4018186-.0121814z" />
										<path id="Shape" d="m58 50h-7.321c-.8158107.0000582-1.5498868-.4953938-1.855-1.252l-8.415-19.336c-1.8483621-4.4927018-6.2299429-7.4216589-11.088-7.412h-4.321v-9.01c.8467921-.0137078 1.5964789-.5506779 1.882-1.348.2895315-.8011288.0370459-1.69769388-.628-2.23l-10.933-8.942c-.7692643-.62644514-1.8727357-.62644514-2.642 0l-10.933 8.942c-.66504592.53230612-.91753151 1.4288712-.628 2.23.28552106.7973221 1.03520789 1.3342922 1.882 1.348v17.01h-1c-1.1045695 0-2 .8954305-2 2v3c0 1.1045695.8954305 2 2 2v21c0 1.1045695.8954305 2 2 2h3c1.1045695 0 2-.8954305 2-2v-1h10v1c0 1.1045695.8954305 2 2 2h3c1.1045695 0 2-.8954305 2-2v-21c1.1045695 0 2-.8954305 2-2v-3h1.321c.8158107-.0000582 1.5498868.4953938 1.855 1.252l8.415 19.336c1.8483621 4.4927018 6.2299429 7.4216589 11.088 7.412h7.321c1.1045695 0 2-.8954305 2-2v-6c0-1.1045695-.8954305-2-2-2zm-44.054-47.982c.032-.024.076-.024.108 0l10.879 8.982-21.921-.041zm-8.946 10.982h18v17h-3v-5c0-3.3137085-2.6862915-5.9999999-6-5.9999999s-5.99999995 2.6862914-6 5.9999999v5h-3zm13 17h-8v-5c0-2.209139 1.790861-4 4-4s4 1.790861 4 4zm-14 28v-21h3v4 17zm15-21v3h-10v-3zm0 8h-10v-3h10zm-10 2h10v3h-10zm0 8v-3h10v3zm15 3h-3v-21h3zm2-23h-24v-3h24zm32 23.0000131-7.321-.0000131c-4.0544237.0065418-7.7095249-2.4413918-9.247-6.193l-8.415-19.336c-.6160164-1.4976981-2.0765662-2.4741642-3.696-2.471h-4.321v-6.0000131l4.321.0000131c4.0544237-.0065418 7.7095249 2.4413918 9.247 6.193l8.415 19.336c.6160164 1.4976981 2.0765662 2.4741642 3.696 2.471h7.321z" />
									</g>
								</svg>
								<h5>ECA and CCA</h5>
							</li>
							<li>
								<svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 512.003 512.003" style="enable-background: new 0 0 512.003 512.003;" xml:space="preserve">
									<g>
										<g>
											<path d="M471.636,40.985c-23.233-23.233-54.064-36.829-89.16-39.318c-31.551-2.237-64.326,4.6-95.245,19.812
	c-3.885,1.911-5.366,6.7-3.314,10.512l0.02,0.036c1.952,3.627,6.428,5.112,10.124,3.294c28.462-14,58.505-20.3,87.325-18.257
	c31.334,2.223,58.769,14.269,79.337,34.836c54.773,54.773,44.436,154.233-23.046,221.714
	c-67.479,67.48-166.941,77.817-221.713,23.046c-26.326-26.325-38.555-63.73-34.434-105.323
	c4.177-42.168,24.589-83.502,57.478-116.39c8.891-8.891,18.5-16.949,28.639-24.023c3.399-2.371,4.268-7.035,2.021-10.518
	c-2.346-3.636-7.251-4.652-10.802-2.176c-10.901,7.601-21.227,16.256-30.774,25.803c-35.404,35.404-57.396,80.075-61.925,125.783
	c-4.302,43.431,7.593,82.934,33.582,112.142l-77.123,77.123c-3.427-3.328-7.926-5.167-12.713-5.167
	c-4.878,0-9.466,1.9-12.916,5.35l-58.324,58.324c-3.014,3.014-3.014,7.901,0,10.915c3.014,3.014,7.901,3.014,10.914,0
	l58.324-58.324c0.72-0.721,1.563-0.829,2-0.829c0.437,0,1.28,0.108,2.001,0.829l10.53,10.53c1.103,1.104,1.104,2.899,0,4.003
	l-91.026,91.026c-0.72,0.721-1.563,0.829-2,0.829s-1.28-0.108-2.001-0.829l-10.53-10.53c-1.105-1.105-1.105-2.898,0-4.004
	l8.754-8.753c3.014-3.014,3.014-7.901,0-10.915c-3.014-3.014-7.9-3.014-10.915,0l-8.755,8.754
	C-1.152,477.413-1.152,489,5.97,496.122l10.236,10.237c3.237,3.237,7.509,5.336,12.08,5.61c5.297,0.317,10.331-1.6,14.047-5.316
	l91.026-91.026c7.056-7.057,7.109-18.493,0.181-25.631l77.128-77.128c25.295,22.506,58.306,34.449,94.907,34.449
	c5.663,0,11.412-0.286,17.23-0.862c45.709-4.528,90.379-26.52,125.784-61.925c35.403-35.404,57.395-80.075,61.924-125.783
	C515.099,112.462,501.292,70.641,471.636,40.985z" />
										</g>
									</g>
									<g>
										<g>
											<path d="M445.149,67.471c-21.573-21.573-52.685-31.548-87.58-28.091c-37.114,3.677-73.69,21.837-102.989,51.136
	c-29.298,29.298-47.458,65.873-51.136,102.988c-3.459,34.906,6.518,66.009,28.09,87.581
	c18.772,18.771,44.758,28.762,74.188,28.762c4.392,0,8.861-0.222,13.393-0.672c37.113-3.677,73.689-21.837,102.988-51.136
	c29.299-29.299,47.459-65.874,51.136-102.988C476.696,120.147,466.72,89.044,445.149,67.471z M439.413,84.121
	c6.949,8.502,12.072,18.455,15.245,29.48l-22.362-22.362L439.413,84.121z M266.216,174.279l30.606-30.606l30.605,30.606
	l-30.606,30.606L266.216,174.279z M285.906,215.8l-30.608,30.608l-30.606-30.606l30.608-30.608L285.906,215.8z M307.737,132.757
	l30.6-30.6l30.606,30.605l-30.6,30.6L307.737,132.757z M338.341,185.194l30.603,30.603l-30.606,30.606L307.735,215.8
	L338.341,185.194z M349.258,174.277l30.6-30.6l30.603,30.602l-30.6,30.6L349.258,174.277z M390.772,132.763l30.608-30.608
	l30.602,30.602l-30.608,30.609L390.772,132.763z M428.493,73.213l-7.112,7.113l-22.338-22.338
	C410.055,61.161,420,66.271,428.493,73.213z M379.86,60.634l30.605,30.605l-30.609,30.608l-30.604-30.605L379.86,60.634z
	 M359.09,54.741c1.759-0.174,3.502-0.302,5.235-0.4l-25.988,25.987l-16.453-16.453C334.026,59.084,346.535,55.985,359.09,54.741z
	 M306.919,70.738l20.505,20.505l-30.601,30.599l-25.75-25.75C282.18,85.894,294.259,77.375,306.919,70.738z M260.157,107.009
	l25.75,25.75l-30.606,30.606l-20.503-20.503C241.435,130.199,249.956,118.117,260.157,107.009z M218.804,195.028
	c1.244-12.554,4.341-25.06,9.131-37.2l16.451,16.451L218.4,200.265C218.497,198.531,218.629,196.787,218.804,195.028z
	 M237.268,264.437c-6.947-8.5-12.069-18.449-15.243-29.471l22.357,22.356L237.268,264.437z M248.184,275.35l7.115-7.114
	l22.354,22.354C266.632,287.419,256.683,282.296,248.184,275.35z M296.815,287.924l-30.603-30.603l30.608-30.608l30.603,30.603
	L296.815,287.924z M317.593,293.816c-1.761,0.175-3.508,0.307-5.244,0.403l25.989-25.989l16.454,16.454
	C342.651,289.475,330.146,292.572,317.593,293.816z M369.758,277.822l-20.506-20.506l30.606-30.606l25.753,25.753
	C394.503,262.663,382.42,271.184,369.758,277.822z M416.528,241.545l-25.753-25.752l30.6-30.6l20.507,20.507
	C435.246,218.36,426.726,230.439,416.528,241.545z M457.879,153.53c-1.244,12.555-4.342,25.063-9.135,37.204l-16.455-16.455
	l25.993-25.993C458.184,150.022,458.053,151.769,457.879,153.53z" />
										</g>
									</g>
									<g>
										<g>
											<path d="M148.883,34.406h-17.474L32.333,0.418c-2.357-0.809-4.962-0.429-6.989,1.02c-2.029,1.449-3.233,3.788-3.233,6.281v136.354
	c0.001,2.491,1.205,4.83,3.233,6.279c1.325,0.947,2.897,1.437,4.485,1.437c0.841,0,1.688-0.138,2.504-0.418l99.076-33.989h17.474
	c22.876,0,41.487-18.611,41.487-41.488C190.369,53.017,171.759,34.406,148.883,34.406z M37.547,18.524l84.857,29.112v20.54H37.547
	V18.524z M122.405,104.153l-84.858,29.112V83.612h84.858V104.153z M148.884,101.947h-11.044V49.842h11.043
	c14.366,0,26.052,11.688,26.052,26.052C174.935,90.259,163.249,101.947,148.884,101.947z" />
										</g>
									</g>
									<g></g>
									<g></g>
									<g></g>
									<g></g>
									<g></g>
									<g></g>
									<g></g>
									<g></g>
									<g></g>
									<g></g>
									<g></g>
									<g></g>
									<g></g>
									<g></g>
									<g></g>
								</svg>

								<h5>ECA and CCA</h5>
							</li>
							<li>
								<svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 480 480" xml:space="preserve">
									<g>
										<g>
											<path d="M339.904,161.776l-5.136-16.904c-6.524-19.202-27.379-29.479-46.581-22.955c-10.802,3.67-19.285,12.153-22.955,22.955
	l-5.136,16.904c-4.322,14.256-5.229,29.329-2.648,44c2.354,12.352,10.196,22.964,21.312,28.84L273.008,363.2
	c-0.77,7.177,1.558,14.342,6.4,19.696c10.413,11.39,28.089,12.182,39.479,1.769c0.616-0.563,1.206-1.153,1.769-1.769
	c4.734-5.202,7.058-12.157,6.4-19.16L321.272,234.6c11.101-5.879,18.93-16.484,21.28-28.824
	C345.133,191.105,344.226,176.032,339.904,161.776z M326.792,202.992c-1.732,8.874-8.056,16.155-16.6,19.112
	c-3.243,1.219-5.332,4.387-5.176,7.848L311.096,365c0.28,2.649-0.594,5.29-2.4,7.248c-4.817,4.769-12.575,4.769-17.392,0
	c-1.896-2.116-2.757-4.964-2.352-7.776l6.032-134.52c0.156-3.461-1.933-6.629-5.176-7.848c-8.544-2.957-14.868-10.238-16.6-19.112
	c-2.14-12.18-1.386-24.693,2.2-36.528l5.136-16.912c3.974-10.745,15.906-16.234,26.652-12.26c5.681,2.101,10.159,6.58,12.26,12.26
	l5.136,16.912C328.174,178.3,328.928,190.812,326.792,202.992z" />
										</g>
									</g>
									<g>
										<g>
											<path d="M216,120c-4.418,0-8,3.582-8,8v68.688l-21.656,21.656c-1.602,1.596-2.454,3.797-2.344,6.056l7,140
	c0.167,3.019-0.929,5.972-3.024,8.152c-4.379,4.418-11.511,4.45-15.93,0.07c-0.024-0.023-0.047-0.047-0.07-0.07
	c-2.096-2.18-3.191-5.133-3.024-8.152l7.048-140c0.112-2.258-0.737-4.458-2.336-6.056L152,196.688V128c0-4.418-3.582-8-8-8
	s-8,3.582-8,8v72c0,2.122,0.844,4.156,2.344,5.656l21.488,21.496l-6.824,136.472c-0.726,14.907,10.77,27.581,25.677,28.307
	c14.907,0.726,27.581-10.77,28.307-25.677c0.043-0.876,0.043-1.753,0-2.629l-6.824-136.472l21.488-21.496
	c1.5-1.5,2.344-3.534,2.344-5.656v-72C224,123.582,220.418,120,216,120z" />
										</g>
									</g>
									<g>
										<g>
											<path d="M168,120c-4.418,0-8,3.582-8,8v64c0,4.418,3.582,8,8,8s8-3.582,8-8v-64C176,123.582,172.418,120,168,120z" />
										</g>
									</g>
									<g>
										<g>
											<path d="M192,120c-4.418,0-8,3.582-8,8v64c0,4.418,3.582,8,8,8s8-3.582,8-8v-64C200,123.582,196.418,120,192,120z" />
										</g>
									</g>
									<g>
										<g>
											<path d="M240,0C107.452,0,0,107.452,0,240s107.452,240,240,240s240-107.452,240-240C479.85,107.514,372.486,0.15,240,0z M240,464
	C116.288,464,16,363.712,16,240S116.288,16,240,16s224,100.288,224,224C463.859,363.653,363.653,463.859,240,464z" />
										</g>
									</g>
									<g>
										<g>
											<path d="M265.969,390.172c-0.429-4.397-4.342-7.614-8.74-7.185c-0.103,0.01-0.206,0.022-0.309,0.036l0.032-0.008
	c-11.263,1.308-22.641,1.308-33.904,0c-4.39-0.515-8.365,2.626-8.88,7.016c-0.515,4.39,2.626,8.365,7.016,8.88
	c12.491,1.459,25.109,1.459,37.6,0C263.181,398.483,266.398,394.57,265.969,390.172z" />
										</g>
									</g>
									<g>
										<g>
											<path d="M362.4,136.912c-2.839-3.385-7.885-3.828-11.27-0.988c-0.011,0.009-0.022,0.019-0.034,0.028
	c-3.377,2.848-3.807,7.894-0.96,11.272c48.235,57.26,44.545,141.929-8.488,194.776c-3.13,3.119-3.139,8.186-0.02,11.316
	c3.119,3.13,8.186,3.139,11.316,0.02C411.881,294.623,415.991,200.541,362.4,136.912z" />
										</g>
									</g>
									<g>
										<g>
											<g>
												<path d="M186.392,105.744c-0.005,0-0.01-0.001-0.015-0.001c-0.008,0-0.016,0.001-0.025,0.001H186.392z" />
												<path d="M277.736,84.472c-31.265-7.493-64.055-5.479-94.168,5.784c-4.136,1.555-6.227,6.169-4.672,10.304
		c1.172,3.117,4.152,5.18,7.481,5.183c0.957-0.005,1.905-0.18,2.799-0.519c27.118-10.141,56.646-11.952,84.8-5.2
		c4.295,1.038,8.618-1.601,9.656-5.896C284.67,89.833,282.031,85.51,277.736,84.472z" />
											</g>
										</g>
									</g>
									<g>
										<g>
											<path d="M137.992,341.6C111.023,314.677,95.908,278.108,96,240c-0.079-30.446,9.565-60.122,27.528-84.704
	c2.655-3.532,1.944-8.547-1.588-11.202s-8.547-1.944-11.202,1.588c-0.047,0.063-0.093,0.126-0.138,0.19
	c-46.253,63.588-39.447,151.359,16.056,207.056c3.128,3.13,8.202,3.132,11.332,0.004C141.118,349.804,141.12,344.73,137.992,341.6
	z" />
										</g>
									</g>
									<g></g>
									<g></g>
									<g></g>
									<g></g>
									<g></g>
									<g></g>
									<g></g>
									<g></g>
									<g></g>
									<g></g>
									<g></g>
									<g></g>
									<g></g>
									<g></g>
									<g></g>
								</svg>
								<h5>Hygienic Meals</h5>
							</li>
						</ul> -->
						<div class="features-more text-center mb-5">
							<a href="" class="btn btn-primary rounded-pill my-2">View All Facilities</a>
						</div>
					</div>
				</div>
			</div>
		</div>
	</div>
<?php endif; ?>
<!--School Features-->

<!--School News-->

<div class="section news-events bg-light pt-60 pb-60">
	<div class="container">
		<div class="row">
			<div class="col-md-8">
				<h1 class="mb-4">School News</h1>
				<div class="news-block">
					<div class="row">
						<div class="col-md-4 col-sm-4 pb-3">
							<img class="img-responsive" width="100%" src="<?php bloginfo('template_directory') ?>/img/n1.jpg" alt="Card image cap" />
						</div>
						<div class="col-md-8 col-sm-8 event-block">
							<h5>Farewell Program 2018</h5>
							<div class="news-date">
								<span><i class="far fa-calendar-alt"></i> 2 minutes ago
								</span>
								<span><i class="far fa-user"></i> 2 minutes ago </span>
								<span><i class="fas fa-list"></i> 2 minutes ago </span>
							</div>
							<p>
								Lorem ipsum dolor sit amet, consectetur adipisicing elit.
								Iure voluptate dolorum ipsa illo suscipit quaerat rem natus!
								Sapiente illum error molestias quidem explicabo deserunt
								enim! Sapiente illum error molestias quidem explicabo
								deserunt enim! [...]
							</p>
							<!-- <a href="#">View Event <i class="fas fa-angle-right"></i> </a> -->
						</div>
					</div>
				</div>

				<div class="news-block">
					<div class="row">
						<div class="col-md-4 col-sm-4 pb-3">
							<img class="img-responsive" width="100%" src="<?php bloginfo('template_directory') ?>/img/n1.jpg" alt="Card image cap" />
						</div>
						<div class="col-md-8 col-sm-8 event-block">
							<h5>Farewell Program 2018</h5>
							<div class="news-date">
								<span><i class="far fa-calendar-alt"></i> 2 minutes ago
								</span>
								<span><i class="far fa-user"></i> 2 minutes ago </span>
								<span><i class="fas fa-list"></i> 2 minutes ago </span>
							</div>
							<p>
								Lorem ipsum dolor sit amet, consectetur adipisicing elit.
								Iure voluptate dolorum ipsa illo suscipit quaerat rem natus!
								Sapiente illum error molestias quidem explicabo deserunt
								enim! Sapiente illum error molestias quidem explicabo
								deserunt enim! [...]
							</p>
							<!-- <a href="#">View Event <i class="fas fa-angle-right"></i> </a> -->
						</div>
					</div>
				</div>

				<div class="news-block">
					<div class="row">
						<div class="col-md-4 col-sm-4 pb-3">
							<img class="img-responsive" width="100%" src="<?php bloginfo('template_directory') ?>/img/n1.jpg" alt="Card image cap" />
						</div>
						<div class="col-md-8 col-sm-8 event-block">
							<h5>Farewell Program 2018</h5>
							<div class="news-date">
								<span><i class="far fa-calendar-alt"></i> 2 minutes ago
								</span>
								<span><i class="far fa-user"></i> 2 minutes ago </span>
								<span><i class="fas fa-list"></i> 2 minutes ago </span>
							</div>
							<p>
								Lorem ipsum dolor sit amet, consectetur adipisicing elit.
								Iure voluptate dolorum ipsa illo suscipit quaerat rem natus!
								Sapiente illum error molestias quidem explicabo deserunt
								enim! Sapiente illum error molestias quidem explicabo
								deserunt enim! [...]
							</p>
							<!-- <a href="#">View Event <i class="fas fa-angle-right"></i> </a> -->
						</div>
					</div>
				</div>

				<div class="news-block">
					<div class="row">
						<div class="col-md-4 col-sm-4 pb-3">
							<img class="img-responsive" width="100%" src="<?php bloginfo('template_directory') ?>/img/n1.jpg" alt="Card image cap" />
						</div>
						<div class="col-md-8 col-sm-8 event-block">
							<h5>Farewell Program 2018</h5>
							<div class="news-date">
								<span><i class="far fa-calendar-alt"></i> 2 minutes ago
								</span>
								<span><i class="far fa-user"></i> 2 minutes ago </span>
								<span><i class="fas fa-list"></i> 2 minutes ago </span>
							</div>
							<p>
								Lorem ipsum dolor sit amet, consectetur adipisicing elit.
								Iure voluptate dolorum ipsa illo suscipit quaerat rem natus!
								Sapiente illum error molestias quidem explicabo deserunt
								enim! Sapiente illum error molestias quidem explicabo
								deserunt enim! [...]
							</p>
							<!-- <a href="#">View Event <i class="fas fa-angle-right"></i> </a> -->
						</div>
					</div>
				</div>
			</div>
			<div class="col-md-4 about-description">
				<h1>Calendar</h1>
				<iframe src="<?php bloginfo('template_directory') ?>/https://calendar.google.com/calendar/embed?height=600&amp;wkst=1&amp;bgcolor=%23ffffff&amp;ctz=Asia%2FKathmandu&amp;src=aW5lcGFsY3JlYXRpb25AZ21haWwuY29t&amp;src=YWRkcmVzc2Jvb2sjY29udGFjdHNAZ3JvdXAudi5jYWxlbmRhci5nb29nbGUuY29t&amp;color=%23039BE5&amp;color=%2333B679&amp;showPrint=0&amp;showDate=1&amp;showNav=1&amp;mode=MONTH&amp;showTabs=0&amp;showCalendars=0&amp;showTz=0&amp;showTitle=0" style="border-width: 0;" width="100%" height="600" frameborder="0" scrolling="no"></iframe>
			</div>
		</div>
	</div>
</div>
<!--About Us-->

<!--Testimonials-->
<div class="section owl-testimonial owl-carousel owl-theme">
	<div class="testimonials pt-60 pb-60">
		<div class="container">
			<div class="row">
				<div class="single-testimonial my-auto">
					<div class="col-lg-12 clear-fix">
						<blockquote>
							<p>
								During the leisure time, after the completion of my SLC, I
								came to Dharan from Dhankuta and became a part of 15 days
								Personality Development Training Course. During the course,
								facilitated by National Academy, I came to know myself which
								furthermore grew along with the studies in the college.
								Apart from good academics, college also provides
								extra-curricular activities to the students for their
								overall performance which differentiates National Academy
								from rest of the colleges.
							</p>
						</blockquote>
					</div>

					<div class="col-sm-6 float-left testimonial-img">
						<img src="<?php bloginfo('template_directory') ?>/img/testimonial1.jpg" class="rounded-circle" alt="" />
					</div>
					<div class="col-sm-6 float-right testimonial-info">
						<h4>Rabina Baraily</h4>
						<h5>No Marks Face of the College</h5>
					</div>
				</div>
			</div>
		</div>
	</div>
	<div class="testimonials pt-60 pb-60">
		<div class="container">
			<div class="row">
				<div class="single-testimonial my-auto">
					<div class="col-lg-12 clear-fix">
						<blockquote>
							<p>
								NASS has provided me the platform to participate in 'Face of
								Nepal‘ , a nationwide beauty & brain competiton, where l was
								awarded with the title ‘Miss Beautiful Eyes’. I am debted to
								the college for not only providing quality education but
								also for all the extra curricular activities, students are
								being engaged.
							</p>
						</blockquote>
					</div>

					<div class="col-sm-6 float-left testimonial-img">
						<img src="<?php bloginfo('template_directory') ?>/img/testimonial2.jpg" class="rounded-circle" alt="" />
					</div>
					<div class="col-sm-6 float-right testimonial-info">
						<h4>Rabina Baraily</h4>
						<h5>No Marks Face of the College</h5>
					</div>
				</div>
			</div>
		</div>
	</div>
</div>
<!--Testimonials-->
<?php
get_footer();
?>